...
}: { step: number; label: string; sublabel: string; status: "done" | "active" | "pending" }) {
  return (
...
export default function ClientBookingDetailPage({ params }: Props) {
  ...
  const step1Status = (depositVerified ? "done" : "active") as "done" | "active" | "pending"
  const step2Status = (depositVerified ? (totalInst === 0 ? "active" : "done") : "pending") as "done" | "active" | "pending"
